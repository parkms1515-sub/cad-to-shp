...
# --- 단계 변경 감지 및 강제 상단 스크롤 (전역 적용) ---
if st.session_state.last_step != st.session_state.current_step:
                        ...
                        if line_count > 0 and point_count == 0:
                            ...
                            with col2:
                                with st.popover("🔍 유형변환 ", use_container_width=True):
                                    st.image(os.path.join(os.path.dirname(__file__), "type_a.png"), use_container_width=True)
                                    st.markdown("<div style='margin-bottom: 15px;'></div>", unsafe_allow_html=True)
                                    st.image(os.path.join(os.path.dirname(__file__), "type_b.png"), use_container_width=True)
                            ...
                            with col1:
                                st.markdown("<p style='font-size: 1.1rem; font-weight: bold; padding-top: 5px; white-space: nowrap;'>변환 방식 선택</p>", unsafe_allow_html=True)
                            with col2:
                                with st.popover("🔍 유형 확인", use_container_width=True):
                                    st.image(os.path.join(os.path.dirname(__file__), "type_c.png"), use_container_width=True)
                                    st.markdown("<div style='margin-bottom: 15px;'></div>", unsafe_allow_html=True)
                                    st.image(os.path.join(os.path.dirname(__file__), "type_d.png"), use_container_width=True)
                        ...
                        for e in current_entities:
                            try:
                                ...
                                if etype == 'LINE':
                                    ...
                                    # 1. 캐드에서 '닫힘(폐합)' 속성이 켜져있는 경우 무조건 연결
                                    if e.is_closed and len(pts) > 0:
                                        pts.append(pts[0])
                                    # 2. 속성은 꺼져있어도 시작점과 끝점이 10.0 (허용 오차) 이내로 매우 가깝다면 폐합으로 간주
                                    elif len(pts) >= 3:
                                        p1, p2 = pts[0], pts[-1]
...
# [단계 2: 좌표 설정 - 9개 PRJ 데이터 및 요청 매핑 반영]
    ...
    with col_guide_img:
        with st.popover("📌 GPS 측량기 화면 확인", use_container_width=True):
            st.image(os.path.join(os.path.dirname(__file__), "coord_guide.png"), caption="GPS 측량기 좌표 설정 화면 참고", use_container_width=True)
...
# [단계 3: 지도 오버레이 및 편집]
    ...
    st.write(" ")
    m_col1, m_col2 = st.columns([4.2, 0.8])
    with m_col1:
        # --- [신규] 주소 자동 조회 로직 ---
        if 'detected_address' not in st.session_state:
            render_data = st.session_state.get('final_render_data', [])
            if render_data:
                try:
                    # 모든 포인트 추출
                    all_x = [p['x'] for ent in render_data for p in ent['points']]
                    all_y = [p['y'] for ent in render_data for p in ent['points']]
                    ...
                    if all_x and all_y:
                        ...
                        from pyproj import Transformer
                        temp_transformer = Transformer.from_crs(f"EPSG:{src_epsg}", "EPSG:4326", always_xy=True)
                        lon, lat = temp_transformer.transform(center_x, center_y)
                        
                        # 브이월드 Reverse Geocoding API 호출
                        vworld_key = "91B25BFE-3424-398D-926F-741A1F1344E5"
                        addr_url = f"http://api.vworld.kr/req/address?service=address&request=getAddress&key={vworld_key}&type=PARCEL&point={lon},{lat}"
                        res = requests.get(addr_url, timeout=5)
                        if res.status_code == 200:
                            data = res.json()
                            results = data.get('response', {}).get('result', [])
                            address = results[0].get('text', '주소 검색 결과 없음') if results else '주소 검색 결과 없음'
                            st.session_state.detected_address = address
                        else:
                            st.session_state.detected_address = "주소 조회 실패"
                    else:
                        st.session_state.detected_address = "분석 대상 없음"
                except Exception as e:
                    st.session_state.detected_address = f"검색 중 오류"
            else:
                st.session_state.detected_address = "분석 대상 데이터 없음"

        addr_text = st.session_state.get('detected_address', '분석 중...')
        if addr_text and addr_text not in ['분석 중...', '분석 대상 없음', '분석 대상 데이터 없음', '주소 조회 실패', '검색 중 오류', '주소 검색 결과 없음']:
            addr_text = f"{addr_text} 인근"
        st.markdown(f"### 🌍 배경 지도 <span style='font-size: 0.95rem; font-weight: normal; color: #4A90E2; display: inline-block; margin-left: 10px;'>📍 분석 위치: <b>{addr_text}</b></span>", unsafe_allow_html=True)
    
    with m_col2:
        if 'split_view' not in st.session_state:
            st.session_state.split_view = False
        if st.button("🖥️ 화면 분할", use_container_width=True, type="secondary"):
            st.session_state.split_view = not st.session_state.split_view
            st.rerun()

    selected_mode = "cadastral" # 기본값 고정
    ...
    try:
        ...
        def create_leaflet_js(map_id, mode="cadastral"):
            ...
            # 레이어 정의
            layers = {
                "base": f"L.tileLayer('http://api.vworld.kr/req/wmts/1.0.0/{vworld_key}/Base/{{z}}/{{y}}/{{x}}.png')",
                "sat": f"L.tileLayer('http://api.vworld.kr/req/wmts/1.0.0/{vworld_key}/Satellite/{{z}}/{{y}}/{{x}}.jpeg')",
                "hybrid": f"L.tileLayer('http://api.vworld.kr/req/wmts/1.0.0/{vworld_key}/Hybrid/{{z}}/{{y}}/{{x}}.png', {{opacity: 0.8}})",
                "cadastral": f"""
                    L.tileLayer.wms('http://api.vworld.kr/req/wms', {{
            ...
            if mode == "satellite":
                tile_layers = f"{layers['sat']}.addTo(map); {layers['hybrid']}.addTo(map);"
            elif mode == "street":
                tile_layers = f"{layers['base']}.addTo(map);"
            elif mode == "cadastral":
                tile_layers = f"{layers['base']}.addTo(map); {layers['cadastral']}.addTo(map);"
            elif mode == "sat_cadastral":
                tile_layers = f"{layers['sat']}.addTo(map); {layers['hybrid']}.addTo(map); {layers['cadastral']}.addTo(map);"
            else: # 복합 모드나 기본값
                tile_layers = f"{layers['base']}.addTo(map); {layers['cadastral']}.addTo(map);"
        ...
        if st.session_state.split_view:
            ...
            with col_right:
                st.caption("🏙️ 분할 화면 2: 브이월드 배경지도")
                components.html(leaflet_head + create_leaflet_js("map_street", "street"), height=570)
        else:
            components.html(leaflet_head + create_leaflet_js("map_full", selected_mode), height=570)
    ...
    # ⓒ 유형 선택 시 -> 3.4 포인트(측량) 변환만 활성화
        ...
        if not survey_pts: 
            ...
            # 사이드바 완료 표시 로직
            if st.session_state.get('btn_3_4_downloaded', False):
                st.sidebar.success("✅ 3.4 작업 완료(다운로드 폴더확인)")
            ...
            if is_poly_done:
                ...
                with col_dl:
                    ...
                    st.download_button(
                        ...
                        mime="application/zip",
                        key="survey_poly_dl",
                        on_click=lambda: st.session_state.update({"btn_3_4_downloaded": True}),
                        use_container_width=True,
                        type="primary" if not st.session_state.get('btn_3_4_downloaded') else "secondary"
                    )
        st.write(" ")
